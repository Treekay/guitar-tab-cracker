...
def execute(plan_path, root):
    ...
    pdf.setTitle("Yuki no Hana - Ryoten Okazaki - reconstructed visual tab")
    ...
    for page_index, rows in enumerate(plan["pages"], 1):
        ...
        draw.text((160, 70), "Yuki no Hana / Ryoten Okazaki", font=font, fill="#202020")
        draw.text((160, 124), f"Source-faithful visual reconstruction | Measures {rows[0][0]}-{rows[-1][-1]} | A4 landscape | {page_index}/{len(plan['pages'])}", font=small, fill="#555555")
